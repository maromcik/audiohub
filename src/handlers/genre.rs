...
use crate::templates::audiobook::{AudiobooksByGenreContentTemplate, AudiobooksByGenreTemplate};
...
async fn get_audiobooks_by_genre(
    ...
    let template = AudiobooksByGenreTemplate::from(
        get_genre_base(u, audiobook_repo, genre_repo, path.into_inner().0).await?,
    );
    let body = template.render()?;
    Ok(HttpResponse::Ok().content_type("text/html").body(body))
}
...
async fn get_audiobooks_by_genre_content(
    ...
    let u = authorized!(identity, request.path());
    let template = AudiobooksByGenreContentTemplate::from(
        get_genre_base(u, audiobook_repo, genre_repo, path.into_inner().0).await?,
    );
    let body = template.render()?;
    Ok(HttpResponse::Ok().content_type("text/html").body(body))
