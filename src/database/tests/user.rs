...
pub mod user_repo_tests {
    ...
    async fn create_user(pool: PgPool) {
        ...
        let u = user_repository.create(&UserCreate::new("pes", "p@p.com", "", "", "", "", "", "")).await.unwrap();
    ...
    async fn update_user(pool: PgPool) {
        let arc_pool = Arc::new(pool);
        let mut user_repository = UserRepository::new(PoolHandler::new(arc_pool));
        let users = user_repository.update(&UserUpdate::new(&10, Some("doggo"), Some("d@d.com"), None, None, None, None, None, None)).await.unwrap();
        let u = &users[0];
